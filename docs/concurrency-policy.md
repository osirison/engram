...
| **Agent `update_memory`**                                 | **Rejects a blind update** — `expectedVersion` is required; omitting it returns a `CONFLICT`-class error explaining the memory must be re-read first.                                        | Conservative / never-lose-data (qp): agent-to-agent overwrites are the multi-writer hazard G4 names. Enforced by **G4-T2**. |
...
| **Lifecycle jobs** (decay, supersede, dedup-link, access) | Route through the version-checked `update()` helper + emit audit where a user-visible mutation occurs.                                                                                       | Prevents a background pass from silently clobbering a concurrent user edit. Enforced by **G3-T3**.                          |
