...
- **Server, database, and dependencies up.** The importer writes through the LTM
  service, which needs Postgres reachable (the only backing service). Bring up
  services and build the workspace:
...
query `project:engram` — for example "how does ENGRAM store vectors" should
surface that pgvector is the only backend, with vectors in the runtime-managed
`embedding_vec` column, imported from the repo `CLAUDE.md`.
