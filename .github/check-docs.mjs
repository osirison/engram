...
const ignoredDirectories = new Set([
  ...
  '__fixtures__',
  // Linked worktrees live at .worktrees/<branch> and contain a full second copy
  // of the tree, including generated docs. Walking into them makes a local run
  // report failures for files that are not in this checkout at all, and CI —
  // which has no worktrees — never reproduces them.
  '.worktrees',
]);
...
const ignoredRelativePaths = [
  ...
  // Agent tooling, not project documentation. These Markdown files are
  // generated by the `openspec` CLI into per-assistant directories and carry
  // that ecosystem's own frontmatter contract (`name`, `description`,
  // `allowed-tools`, `license`) — `title` is not part of it, so requiring one
  // here would both mislabel the files and be overwritten on the next
  // regeneration. Their headings are equally out of scope: the duplicate
  // "Requirement:"/"Scenario:" headings this linter flags are deliberate
  // template examples inside the prompts.
  //
  // Scoped to the generated subtrees rather than all of `.github`, so
  // `.github/copilot-instructions.md` stays policed.
  '.claude',
  '.hermes',
