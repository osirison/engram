...
> The canonical copy of this guide lives in the repository at
> [`docs/agent-memory-sync.md`](https://github.com/osirison/engram/blob/main/docs/agent-memory-sync.md) —
> agents read it from the repo at runtime. This page is a published mirror.

The one-time [migration runbook](/docs/agent-memory/migration/) imports existing
...
is `userId: "qp"`; scope follows the [contract](/docs/agent-memory/contract/).
