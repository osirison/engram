...
- Multi-tenant (`standard`) + `MCP_TRANSPORT=streamable-http` +
...
### `lite` — single-user, Postgres-backed
...
pgvector is the only vector backend; unset values use pgvector's
defaults.
...
`standard`-profile-only, Postgres-backed fixed window; all buckets are
evaluated and the tightest one wins (`429` + `Retry-After`).
