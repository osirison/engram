...
| A07 | Identification & Authentication Failures | ✅ Pass    | Admin token required for all destructive admin tools. API key management with proper CRUD. JWT secret configurable. Rate limiting implemented on the `standard` profile (Postgres-backed fixed window).                                                                                                                   |
...
| A10 | Server-Side Request Forgery (SSRF)       | ✅ Pass    | No user-controlled URL fetching. External calls (OpenAI, Ollama, Postgres) are to operator-configured endpoints only.                                                                                                                                                                                                     |
