...
- **STM atomic CAS (G4-T4) — closed.** The Postgres STM port made `update()` a true
  single-statement compare-and-set (the `version` guard rides in the `WHERE` clause),
  eliminating the former read-compare-set window.
