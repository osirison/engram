...
Set `DEPLOYMENT_PROFILE` in `.env.prod`. Both profiles run on the same
...
> **Note:** PostgreSQL is the only backing service `docker-compose.prod.yml`
> provisions. Choosing `standard` over `lite` needs no extra services — see
> [Migrate lite to standard](/docs/how-to/profile-migration/) for the auth
> configuration `standard` requires.
...
1. Builds the image and smoke-tests it (boots the `lite` profile against a
   throwaway Postgres and polls `/health`) before anything is published.
