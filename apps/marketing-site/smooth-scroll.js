...
if (!reduce && !coarse) {
  ...
  let animating = false;
  let raf = 0;
  const EASE = 0.085; // lower = longer, dreamier glide
  ...
  const kick = () => { if (!raf) { animating = true; raf = requestAnimationFrame(loop); } };
  ...
  window.addEventListener("resize", () => { target = clamp(target); current = clamp(current); });
}
